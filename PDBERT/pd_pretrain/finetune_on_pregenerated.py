...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch}") as pbar:
            for step, batch in enumerate(train_dataloader):
                ...
                outputs = model(input_ids, input_mask, masked_lm_labels=lm_label_ids)
                loss = outputs[0]
                if n_gpu > 1:
                ...
                if global_step % writeout_step == 0 and global_step != 0 and global_step != last_save_step:
        ...
        logging.info("** ** * Saving pretrained model epoch ** ** * ")
        save_checkpoint_dir = Path(f'{args.output_dir}/ep{epoch}/')
        save_checkpoint_dir.mkdir(parents=True, exist_ok=True)
        module = model
        if isinstance(model, torch.nn.DataParallel):
            module = model.module
        module.save_pretrained(save_checkpoint_dir)
    ...
    if n_gpu > 1 or n_gpu <= 1:
        if isinstance(model, torch.nn.DataParallel):
            model = model.module
        logging.info("** ** * Saving pretraining model for ep{} ** ** * ".format(epoch))
