...
def evaluate(args, model, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    if write_pred:
        output_file = os.path.join(args.pre_dir, "predictions.json")
...
def train_absa(args, model, train_examples, train_features, train_dataloader,
    ...
    for epoch in range(epochs):
        ...
        metrics = evaluate(args, model, eval_examples, eval_features, eval_dataloader, logger, write_pred=False)
...
def main():
    ...
    parser.add_argument('--pre_dir', type=Path)
    ...
    train_examples, train_features, train_dataloader = read_train_data(args, tokenizer, logger)
    da_train_examples, da_train_features, da_train_dataloader = read_da_train_data(args, tokenizer, logger)
    
    logger.info("***** Preparing evaluation *****")
    eval_examples, eval_features, eval_dataloader = read_eval_data(args, tokenizer, logger)

    da_train_absa(args, model, da_train_examples, da_train_features, da_train_dataloader, eval_examples, eval_features, eval_dataloader)
